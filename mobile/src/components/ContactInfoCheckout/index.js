...
const castToOrderItems = (products = []) => products.map(({product, quantity}) =>( {product: product.id, quantity}));
const getTotal = (products = []) => products.reduce((prev, {product}) => prev + product.price, 0);
...
export default function ContactInfoCheckout({products}) {
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const [{ transaction }, removeCartsCount] = await Promise.all([
        checkoutPromise,
        removeCartsPromise,
      ]);
